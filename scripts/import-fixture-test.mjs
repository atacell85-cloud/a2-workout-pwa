...
const chrome = spawn(chromePath, ['--headless=new', '--disable-gpu', '--no-first-run', '--remote-debugging-port=9225', `--user-data-dir=${userDataDir}`, `http://localhost:${port}`], { stdio: 'ignore' });
...
async function waitForWs() { for (let i = 0; i < 50; i += 1) { try { const targets = await (await fetch('http://127.0.0.1:9225/json/list')).json(); const page = targets.find(target => target.type === 'page' && target.url.startsWith(`http://localhost:${port}`)); if (page) return page.webSocketDebuggerUrl; } catch {} await wait(100); } throw new Error('Chrome CDP unavailable'); }
